...
$regioes_add = NULL; // Initialize page object first

class cregioes_add extends cregioes {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["id_regiao"] != "") {
				...
				$this->setKey("id_regiao", $this->id_regiao->CurrentValue); // Set up key
			} else {
				$this->setKey("id_regiao", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("regioeslist.php"); // No matching record, return to list
				}
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "regioesview.php")
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("id_regiao")) <> "")
			$this->id_regiao->CurrentValue = $this->getKey("id_regiao"); // id_regiao
		else
			$bValidKey = FALSE;
...
// Create page object
if (!isset($regioes_add)) $regioes_add = new cregioes_add();

// Page init
$regioes_add->Page_Init();

// Page main
$regioes_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$regioes_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var regioes_add = new ew_Page("regioes_add");
regioes_add.PageID = "add"; // Page ID
var EW_PAGE_ID = regioes_add.PageID; // For backward compatibility

// Form object
var fregioesadd = new ew_Form("fregioesadd");

// Validate form
fregioesadd.Validate = function() {
...
fregioesadd.Form_CustomValidate = 
...
fregioesadd.ValidateRequired = true;
<?php } else { ?>
fregioesadd.ValidateRequired = false; 
<?php } ?>
...
<?php $regioes_add->ShowPageHeader(); ?>
<?php
$regioes_add->ShowMessage();
?>
<form name="fregioesadd" id="fregioesadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($regioes_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $regioes_add->Token ?>">
<?php } ?>
