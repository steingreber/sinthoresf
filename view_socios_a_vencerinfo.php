...
$view_socios_a_vencer = NULL;

//
// Table class for view_socios_a_vencer
//
class cview_socios_a_vencer extends cTable {
	var $cod_socio;
	var $ativo;
	var $validade;
	var $nome;
	...
	function __construct() {
		...
		$this->TableVar = 'view_socios_a_vencer';
		$this->TableName = 'view_socios_a_vencer';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// cod_socio
		$this->cod_socio = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_cod_socio', 'cod_socio', '`cod_socio`', '`cod_socio`', 3, -1, FALSE, '`cod_socio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cod_socio->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cod_socio'] = &$this->cod_socio;

		// ativo
		$this->ativo = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_ativo', 'ativo', '`ativo`', '`ativo`', 3, -1, FALSE, '`ativo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->ativo->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ativo'] = &$this->ativo;

		// validade
		$this->validade = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_validade', 'validade', '`validade`', 'DATE_FORMAT(`validade`, \'%d/%m/%Y\')', 133, 7, FALSE, '`validade`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->validade->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['validade'] = &$this->validade;

		// nome
		$this->nome = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_nome', 'nome', '`nome`', '`nome`', 200, -1, FALSE, '`nome`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nome'] = &$this->nome;

		// cpf
		$this->cpf = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_cpf', 'cpf', '`cpf`', '`cpf`', 200, -1, FALSE, '`cpf`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cpf->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cpf'] = &$this->cpf;

		// nome_empresa
		$this->nome_empresa = new cField('view_socios_a_vencer', 'view_socios_a_vencer', 'x_nome_empresa', 'nome_empresa', '`nome_empresa`', '`nome_empresa`', 200, -1, FALSE, '`nome_empresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`view_socios_a_vencer`";
	}
	...
	// Update Table
	var $UpdateTable = "`view_socios_a_vencer`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "view_socios_a_vencerlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("view_socios_a_vencerview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("view_socios_a_vencerview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "view_socios_a_venceradd.php?" . $this->UrlParm($parm);
		else
			return "view_socios_a_venceradd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("view_socios_a_venceredit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("view_socios_a_venceradd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("view_socios_a_vencerdelete.php", $this->UrlParm());
	}
