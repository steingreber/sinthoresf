...
$regioes_grid = NULL; // Initialize page object first

class cregioes_grid extends cregioes {
	...
	// Page object name
	var $PageObjName = 'regioes_grid';

	// Grid form hidden field names
	var $FormName = 'fregioesgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["regioes"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "rel_empresas_regioes_") {
			global $rel_empresas_regioes_;
			$rsmaster = $rel_empresas_regioes_->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("rel_empresas_regioes_list.php"); // Return to master page
			} else {
				$rel_empresas_regioes_->LoadListRowValues($rsmaster);
				$rel_empresas_regioes_->RowType = EW_ROWTYPE_MASTER; // Master row
				$rel_empresas_regioes_->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->id_regiao->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->id_regiao->FormValue))
				return FALSE;
		}
		return TRUE;
	}
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_regiao->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_regiao") && $objForm->HasValue("o_regiao") && $this->regiao->CurrentValue <> $this->regiao->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->id_regiao->setSessionValue("");
			}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_regiao->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		$this->regiao->CurrentValue = NULL;
		$this->regiao->OldValue = $this->regiao->CurrentValue;
	}
	...
	function LoadFormValues() {
		...
		$this->regiao->setOldValue($objForm->GetValue("o_regiao"));
		if (!$this->id_regiao->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_regiao->CurrentValue = strval($arKeys[0]); // id_regiao
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "rel_empresas_regioes_") {
				$this->id_regiao->CurrentValue = $this->id_regiao->getSessionValue();
			}
		...
		// regiao
		$this->regiao->SetDbValueDef($rsnew, $this->regiao->CurrentValue, "", FALSE);

		// id_regiao
		if ($this->id_regiao->getSessionValue() <> "") {
			$rsnew['id_regiao'] = $this->id_regiao->getSessionValue();
		}
		...
		if ($AddRow) {
			$this->id_regiao->setDbValue($conn->Insert_ID());
			$rsnew['id_regiao'] = $this->id_regiao->DbValue;
		}
		if ($AddRow) {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "rel_empresas_regioes_") {
			$this->id_regiao->Visible = FALSE;
			if ($GLOBALS["rel_empresas_regioes_"]->EventCancelled) $this->EventCancelled = TRUE;
		}
