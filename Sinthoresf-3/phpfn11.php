...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "csinthore_bdd_db") {
			$file = "sinthore_bdddb.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{31ECB1FF-BE34-4CC5-BF5C-0D3F1718C9D9}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('a03senha'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('a02email')); // Load user name
