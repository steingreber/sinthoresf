...
class cempresas_view extends cempresas {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["cod_empresa"] <> "") {
			$this->RecKey["cod_empresa"] = $_GET["cod_empresa"];
			$KeyUrl .= "&amp;cod_empresa=" . urlencode($this->RecKey["cod_empresa"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["cod_empresa"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["cod_empresa"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["cod_empresa"] <> "") {
				$this->cod_empresa->setFormValue($_POST["cod_empresa"]);
				$this->RecKey["cod_empresa"] = $this->cod_empresa->FormValue;
			} else {
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detailreport_rel_socios_recibo_empresa_");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("rel_socios_recibo_empresa_", "TblCaption");
		...
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_socios_recibo_empresa_');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "rel_socios_recibo_empresa_";
		}
		if ($this->ShowMultipleDetails) $item->Visible = FALSE;

		// "detailreport_rel_func_empresas"
		$item = &$option->Add("detailreport_rel_func_empresas");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("rel_func_empresas", "TblCaption");
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_func_empresas');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "rel_func_empresas";
		}
		if ($this->ShowMultipleDetails) $item->Visible = FALSE;

		// "detailreport_rel_recibo_empresa_"
		$item = &$option->Add("detailreport_rel_recibo_empresa_");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("rel_recibo_empresa_", "TblCaption");
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_recibo_empresa_');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "rel_recibo_empresa_";
		}
		if ($this->ShowMultipleDetails) $item->Visible = FALSE;

		// "detailreport_rel_lista_funcionarios_"
		$item = &$option->Add("detailreport_rel_lista_funcionarios_");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("rel_lista_funcionarios_", "TblCaption");
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_lista_funcionarios_');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "rel_lista_funcionarios_";
		}
		if ($this->ShowMultipleDetails) $item->Visible = FALSE;
	...
	function ExportData() {
		...
		// Export detail records (rel_socios_recibo_empresa_)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("rel_socios_recibo_empresa_", explode(",", $this->getCurrentDetailTable()))) {
			global $rel_socios_recibo_empresa_;
			if (!isset($rel_socios_recibo_empresa_)) $rel_socios_recibo_empresa_ = new crel_socios_recibo_empresa_;
			$rsdetail = $rel_socios_recibo_empresa_->LoadRs($rel_socios_recibo_empresa_->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$rel_socios_recibo_empresa_->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
		...
		// Export detail records (rel_func_empresas)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("rel_func_empresas", explode(",", $this->getCurrentDetailTable()))) {
			global $rel_func_empresas;
			if (!isset($rel_func_empresas)) $rel_func_empresas = new crel_func_empresas;
			$rsdetail = $rel_func_empresas->LoadRs($rel_func_empresas->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$rel_func_empresas->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
		...
		// Export detail records (rel_recibo_empresa_)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("rel_recibo_empresa_", explode(",", $this->getCurrentDetailTable()))) {
			global $rel_recibo_empresa_;
			if (!isset($rel_recibo_empresa_)) $rel_recibo_empresa_ = new crel_recibo_empresa_;
			$rsdetail = $rel_recibo_empresa_->LoadRs($rel_recibo_empresa_->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$rel_recibo_empresa_->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
		...
		// Export detail records (rel_lista_funcionarios_)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("rel_lista_funcionarios_", explode(",", $this->getCurrentDetailTable()))) {
			global $rel_lista_funcionarios_;
			if (!isset($rel_lista_funcionarios_)) $rel_lista_funcionarios_ = new crel_lista_funcionarios_;
			$rsdetail = $rel_lista_funcionarios_->LoadRs($rel_lista_funcionarios_->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				$ExportStyle = $Doc->Style;
				$Doc->SetStyle("h"); // Change to horizontal
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					$Doc->ExportEmptyRow();
					$detailcnt = $rsdetail->RecordCount();
					$rel_lista_funcionarios_->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
				}
				$Doc->SetStyle($ExportStyle); // Restore
				$rsdetail->Close();
			}
...
fempresasview.Lists["x_regiao"] = {"LinkField":"x_id_regiao","Ajax":true,"AutoFill":false,"DisplayFields":["x_regiao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php if (!isset($empresas_view->Pager)) $empresas_view->Pager = new cPrevNextPager($empresas_view->StartRec, $empresas_view->DisplayRecs, $empresas_view->TotalRecs) ?>
...
<?php if ($empresas->cod_empresa->Visible) { // cod_empresa ?>
	<tr id="r_cod_empresa">
		<td><span id="elh_empresas_cod_empresa"><?php echo $empresas->cod_empresa->FldCaption() ?></span></td>
...
<?php echo $empresas->cod_empresa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->nome_empresa->Visible) { // nome_empresa ?>
	<tr id="r_nome_empresa">
		<td><span id="elh_empresas_nome_empresa"><?php echo $empresas->nome_empresa->FldCaption() ?></span></td>
...
<?php echo $empresas->nome_empresa->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->endereco->Visible) { // endereco ?>
	<tr id="r_endereco">
		<td><span id="elh_empresas_endereco"><?php echo $empresas->endereco->FldCaption() ?></span></td>
		<td<?php echo $empresas->endereco->CellAttributes() ?>>
<span id="el_empresas_endereco">
<span<?php echo $empresas->endereco->ViewAttributes() ?>>
<?php echo $empresas->endereco->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->numero->Visible) { // numero ?>
	<tr id="r_numero">
		<td><span id="elh_empresas_numero"><?php echo $empresas->numero->FldCaption() ?></span></td>
		<td<?php echo $empresas->numero->CellAttributes() ?>>
<span id="el_empresas_numero">
<span<?php echo $empresas->numero->ViewAttributes() ?>>
<?php echo $empresas->numero->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->bairro->Visible) { // bairro ?>
	<tr id="r_bairro">
		<td><span id="elh_empresas_bairro"><?php echo $empresas->bairro->FldCaption() ?></span></td>
		<td<?php echo $empresas->bairro->CellAttributes() ?>>
<span id="el_empresas_bairro">
<span<?php echo $empresas->bairro->ViewAttributes() ?>>
<?php echo $empresas->bairro->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->telefone->Visible) { // telefone ?>
	<tr id="r_telefone">
		<td><span id="elh_empresas_telefone"><?php echo $empresas->telefone->FldCaption() ?></span></td>
...
<?php if ($empresas->cidade->Visible) { // cidade ?>
	<tr id="r_cidade">
		<td><span id="elh_empresas_cidade"><?php echo $empresas->cidade->FldCaption() ?></span></td>
		<td<?php echo $empresas->cidade->CellAttributes() ?>>
<span id="el_empresas_cidade">
<span<?php echo $empresas->cidade->ViewAttributes() ?>>
<?php echo $empresas->cidade->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->cgc->Visible) { // cgc ?>
	<tr id="r_cgc">
		<td><span id="elh_empresas_cgc"><?php echo $empresas->cgc->FldCaption() ?></span></td>
		<td<?php echo $empresas->cgc->CellAttributes() ?>>
<span id="el_empresas_cgc">
<span<?php echo $empresas->cgc->ViewAttributes() ?>>
<?php echo $empresas->cgc->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($empresas->regiao->Visible) { // regiao ?>
	<tr id="r_regiao">
		<td><span id="elh_empresas_regiao"><?php echo $empresas->regiao->FldCaption() ?></span></td>
		<td<?php echo $empresas->regiao->CellAttributes() ?>>
<span id="el_empresas_regiao">
<span<?php echo $empresas->regiao->ViewAttributes() ?>>
<?php echo $empresas->regiao->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
