...
class cempresas extends cTable {
	...
	function __construct() {
		...
		// cod_empresa
		$this->cod_empresa = new cField('empresas', 'empresas', 'x_cod_empresa', 'cod_empresa', '`cod_empresa`', '`cod_empresa`', 3, -1, FALSE, '`cod_empresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cod_empresa->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cod_empresa'] = &$this->cod_empresa;

		// nome_empresa
		$this->nome_empresa = new cField('empresas', 'empresas', 'x_nome_empresa', 'nome_empresa', '`nome_empresa`', '`nome_empresa`', 200, -1, FALSE, '`nome_empresa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nome_empresa'] = &$this->nome_empresa;

		// endereco
		$this->endereco = new cField('empresas', 'empresas', 'x_endereco', 'endereco', '`endereco`', '`endereco`', 200, -1, FALSE, '`endereco`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['endereco'] = &$this->endereco;

		// numero
		$this->numero = new cField('empresas', 'empresas', 'x_numero', 'numero', '`numero`', '`numero`', 3, -1, FALSE, '`numero`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->numero->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['numero'] = &$this->numero;

		// bairro
		$this->bairro = new cField('empresas', 'empresas', 'x_bairro', 'bairro', '`bairro`', '`bairro`', 200, -1, FALSE, '`bairro`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['bairro'] = &$this->bairro;

		// telefone
		$this->telefone = new cField('empresas', 'empresas', 'x_telefone', 'telefone', '`telefone`', '`telefone`', 200, -1, FALSE, '`telefone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->telefone->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['telefone'] = &$this->telefone;

		// cidade
		$this->cidade = new cField('empresas', 'empresas', 'x_cidade', 'cidade', '`cidade`', '`cidade`', 200, -1, FALSE, '`cidade`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cidade'] = &$this->cidade;

		// cgc
		$this->cgc = new cField('empresas', 'empresas', 'x_cgc', 'cgc', '`cgc`', '`cgc`', 200, -1, FALSE, '`cgc`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cgc'] = &$this->cgc;

		// regiao
		$this->regiao = new cField('empresas', 'empresas', 'x_regiao', 'regiao', '`regiao`', '`regiao`', 3, -1, FALSE, '`EV__regiao`', TRUE, TRUE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlSelectList() { // Select for List page
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *, (SELECT `regiao` FROM `regioes` `EW_TMP_LOOKUPTABLE` WHERE `EW_TMP_LOOKUPTABLE`.`id_regiao` = `empresas`.`regiao` LIMIT 1) AS `EV__regiao` FROM `empresas`" .
			") `EW_TMP_TABLE`";
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`cod_empresa` DESC";
	}
	...
	function UseVirtualFields() {
		...
		if ($this->BasicSearch->getKeyword() <> "")
			return TRUE;
		if (strpos($sOrderBy, " " . $this->regiao->FldVirtualExpression . " ") !== FALSE)
			return TRUE;
		return FALSE;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("empresasview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("empresasview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "empresasadd.php?" . $this->UrlParm($parm);
		else
			return "empresasadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("empresasedit.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("empresasedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("empresasadd.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("empresasadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->cgc->Exportable) $Doc->ExportCaption($this->cgc);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->cgc->Exportable) $Doc->ExportField($this->cgc);
