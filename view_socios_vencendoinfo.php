...
$view_socios_vencendo = NULL;

//
// Table class for view_socios_vencendo
//
class cview_socios_vencendo extends cTable {
	var $cod_socio;
	var $validade;
	var $ativo;
	...
	function __construct() {
		...
		$this->TableVar = 'view_socios_vencendo';
		$this->TableName = 'view_socios_vencendo';
		$this->TableType = 'VIEW';
		$this->ExportAll = TRUE;
		...
		// cod_socio
		$this->cod_socio = new cField('view_socios_vencendo', 'view_socios_vencendo', 'x_cod_socio', 'cod_socio', '`cod_socio`', '`cod_socio`', 3, -1, FALSE, '`cod_socio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->cod_socio->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['cod_socio'] = &$this->cod_socio;

		// validade
		$this->validade = new cField('view_socios_vencendo', 'view_socios_vencendo', 'x_validade', 'validade', '`validade`', 'DATE_FORMAT(`validade`, \'%d/%m/%Y\')', 133, 7, FALSE, '`validade`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->validade->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['validade'] = &$this->validade;

		// ativo
		$this->ativo = new cField('view_socios_vencendo', 'view_socios_vencendo', 'x_ativo', 'ativo', '`ativo`', '`ativo`', 3, -1, FALSE, '`ativo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`view_socios_vencendo`";
	}
	...
	// Update Table
	var $UpdateTable = "`view_socios_vencendo`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "view_socios_vencendolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("view_socios_vencendoview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("view_socios_vencendoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "view_socios_vencendoadd.php?" . $this->UrlParm($parm);
		else
			return "view_socios_vencendoadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("view_socios_vencendoedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("view_socios_vencendoadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("view_socios_vencendodelete.php", $this->UrlParm());
	}
