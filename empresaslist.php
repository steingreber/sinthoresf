...
class cempresas_list extends cempresas {
	...
	function __construct() {
		...
		$this->AddUrl = "empresasadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function AdvancedSearchWhere($Default = FALSE) {
		...
		$this->BuildSearchSql($sWhere, $this->cod_empresa, $Default, FALSE); // cod_empresa
		$this->BuildSearchSql($sWhere, $this->nome_empresa, $Default, FALSE); // nome_empresa
		...
		if (!$Default && $this->Command == "search") {
			$this->cod_empresa->AdvancedSearch->Save(); // cod_empresa
			$this->nome_empresa->AdvancedSearch->Save(); // nome_empresa
		}
		return $sWhere;
	}
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->cgc, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->regiao, $arKeywords, $type);
		return $sWhere;
	}
	...
	function CheckSearchParms() {
		...
		if ($this->BasicSearch->IssetSession())
			return TRUE;
		if ($this->cod_empresa->AdvancedSearch->IssetSession())
			return TRUE;
		if ($this->nome_empresa->AdvancedSearch->IssetSession())
			return TRUE;
		return FALSE;
	}
	...
	function ResetAdvancedSearchParms() {
		$this->cod_empresa->AdvancedSearch->UnsetSession();
		$this->nome_empresa->AdvancedSearch->UnsetSession();
	}
	...
	function RestoreSearchParms() {
		...
		$this->cod_empresa->AdvancedSearch->Load();
		$this->nome_empresa->AdvancedSearch->Load();
	}
	...
	function SetupListOptions() {
		...
		// "detailreport_rel_socios_recibo_empresa_"
		$item = &$this->ListOptions->Add("detailreport_rel_socios_recibo_empresa_");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_socios_recibo_empresa_') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;

		// "detailreport_rel_func_empresas"
		$item = &$this->ListOptions->Add("detailreport_rel_func_empresas");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_func_empresas') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;

		// "detailreport_rel_recibo_empresa_"
		$item = &$this->ListOptions->Add("detailreport_rel_recibo_empresa_");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_recibo_empresa_') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;

		// "detailreport_rel_lista_funcionarios_"
		$item = &$this->ListOptions->Add("detailreport_rel_lista_funcionarios_");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'rel_lista_funcionarios_') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		$item->ShowInButtonGroup = FALSE;
	...
	function RenderListOptions() {
		...
		// "detailreport_rel_socios_recibo_empresa_"
		$oListOpt = &$this->ListOptions->Items["detailreport_rel_socios_recibo_empresa_"];
		if ($Security->AllowList(CurrentProjectID() . 'rel_socios_recibo_empresa_')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("rel_socios_recibo_empresa_", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink\" href=\"" . ew_HtmlEncode("rel_socios_recibo_empresa_report.php?" . EW_TABLE_SHOW_MASTER . "=empresas&fk_cod_empresa=" . urlencode(strval($this->cod_empresa->CurrentValue)) . "") . "\">" . $body . "</a>";
		...
		// "detailreport_rel_func_empresas"
		$oListOpt = &$this->ListOptions->Items["detailreport_rel_func_empresas"];
		if ($Security->AllowList(CurrentProjectID() . 'rel_func_empresas')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("rel_func_empresas", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink\" href=\"" . ew_HtmlEncode("rel_func_empresasreport.php?" . EW_TABLE_SHOW_MASTER . "=empresas&fk_cod_empresa=" . urlencode(strval($this->cod_empresa->CurrentValue)) . "") . "\">" . $body . "</a>";
			$body = "<div class=\"btn-group\">" . $body . "</div>";
			$oListOpt->Body = $body;
			if ($this->ShowMultipleDetails) $oListOpt->Visible = FALSE;
		}
		...
		// "detailreport_rel_recibo_empresa_"
		$oListOpt = &$this->ListOptions->Items["detailreport_rel_recibo_empresa_"];
		if ($Security->AllowList(CurrentProjectID() . 'rel_recibo_empresa_')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("rel_recibo_empresa_", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink\" href=\"" . ew_HtmlEncode("rel_recibo_empresa_report.php?" . EW_TABLE_SHOW_MASTER . "=empresas&fk_cod_empresa=" . urlencode(strval($this->cod_empresa->CurrentValue)) . "") . "\">" . $body . "</a>";
			$body = "<div class=\"btn-group\">" . $body . "</div>";
			$oListOpt->Body = $body;
			if ($this->ShowMultipleDetails) $oListOpt->Visible = FALSE;
		}
		...
		// "detailreport_rel_lista_funcionarios_"
		$oListOpt = &$this->ListOptions->Items["detailreport_rel_lista_funcionarios_"];
		if ($Security->AllowList(CurrentProjectID() . 'rel_lista_funcionarios_')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("rel_lista_funcionarios_", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink\" href=\"" . ew_HtmlEncode("rel_lista_funcionarios_report.php?" . EW_TABLE_SHOW_MASTER . "=empresas&fk_cod_empresa=" . urlencode(strval($this->cod_empresa->CurrentValue)) . "") . "\">" . $body . "</a>";
			$body = "<div class=\"btn-group\">" . $body . "</div>";
			$oListOpt->Body = $body;
			if ($this->ShowMultipleDetails) $oListOpt->Visible = FALSE;
		}
	...
	function SetupSearchOptions() {
		...
		if (ew_IsMobile())
			$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"empresassrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		else
			$item->Body = "<button type=\"button\" class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" onclick=\"ew_SearchDialogShow({lnk:this,url:'empresassrch.php'});\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"fempresaslistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function LoadSearchValues() {
		...
		$this->cod_empresa->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_cod_empresa"]);
		if ($this->cod_empresa->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->cod_empresa->AdvancedSearch->SearchOperator = @$_GET["z_cod_empresa"];

		// nome_empresa
		$this->nome_empresa->AdvancedSearch->SearchValue = ew_StripSlashes(@$_GET["x_nome_empresa"]);
		if ($this->nome_empresa->AdvancedSearch->SearchValue <> "") $this->Command = "search";
		$this->nome_empresa->AdvancedSearch->SearchOperator = @$_GET["z_nome_empresa"];
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($this->Export == "")
				$this->nome_empresa->ViewValue = ew_Highlight($this->HighlightName(), $this->nome_empresa->ViewValue, $this->BasicSearch->getKeyword(), $this->BasicSearch->getType(), $this->nome_empresa->AdvancedSearch->getValue("x"), "");
			...
			if ($this->Export == "")
				$this->cidade->ViewValue = ew_Highlight($this->HighlightName(), $this->cidade->ViewValue, $this->BasicSearch->getKeyword(), $this->BasicSearch->getType(), "", "");
			...
			if ($this->Export == "")
				$this->regiao->ViewValue = ew_Highlight($this->HighlightName(), $this->regiao->ViewValue, $this->BasicSearch->getKeyword(), $this->BasicSearch->getType(), "", "");
		}
...
fempresaslist.Lists["x_regiao"] = {"LinkField":"x_id_regiao","Ajax":true,"AutoFill":false,"DisplayFields":["x_regiao","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<option value="30"<?php if ($empresas_list->DisplayRecs == 30) { ?> selected="selected"<?php } ?>>30</option>
<option value="50"<?php if ($empresas_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="100"<?php if ($empresas_list->DisplayRecs == 100) { ?> selected="selected"<?php } ?>>100</option>
<option value="ALL"<?php if ($empresas->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php if ($empresas->cod_empresa->Visible) { // cod_empresa ?>
	<?php if ($empresas->SortUrl($empresas->cod_empresa) == "") { ?>
		<th data-name="cod_empresa"><div id="elh_empresas_cod_empresa" class="empresas_cod_empresa"><div class="ewTableHeaderCaption"><?php echo $empresas->cod_empresa->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="cod_empresa"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresas->SortUrl($empresas->cod_empresa) ?>',1);"><div id="elh_empresas_cod_empresa" class="empresas_cod_empresa">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresas->cod_empresa->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($empresas->cod_empresa->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresas->cod_empresa->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($empresas->nome_empresa->Visible) { // nome_empresa ?>
	<?php if ($empresas->SortUrl($empresas->nome_empresa) == "") { ?>
		<th data-name="nome_empresa"><div id="elh_empresas_nome_empresa" class="empresas_nome_empresa"><div class="ewTableHeaderCaption"><?php echo $empresas->nome_empresa->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="nome_empresa"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresas->SortUrl($empresas->nome_empresa) ?>',1);"><div id="elh_empresas_nome_empresa" class="empresas_nome_empresa">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresas->nome_empresa->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($empresas->nome_empresa->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresas->nome_empresa->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($empresas->telefone->Visible) { // telefone ?>
	<?php if ($empresas->SortUrl($empresas->telefone) == "") { ?>
		<th data-name="telefone"><div id="elh_empresas_telefone" class="empresas_telefone"><div class="ewTableHeaderCaption"><?php echo $empresas->telefone->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="telefone"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresas->SortUrl($empresas->telefone) ?>',1);"><div id="elh_empresas_telefone" class="empresas_telefone">
...
<?php if ($empresas->cidade->Visible) { // cidade ?>
	<?php if ($empresas->SortUrl($empresas->cidade) == "") { ?>
		<th data-name="cidade"><div id="elh_empresas_cidade" class="empresas_cidade"><div class="ewTableHeaderCaption"><?php echo $empresas->cidade->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="cidade"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresas->SortUrl($empresas->cidade) ?>',1);"><div id="elh_empresas_cidade" class="empresas_cidade">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresas->cidade->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($empresas->cidade->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresas->cidade->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php if ($empresas->regiao->Visible) { // regiao ?>
	<?php if ($empresas->SortUrl($empresas->regiao) == "") { ?>
		<th data-name="regiao"><div id="elh_empresas_regiao" class="empresas_regiao"><div class="ewTableHeaderCaption"><?php echo $empresas->regiao->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="regiao"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $empresas->SortUrl($empresas->regiao) ?>',1);"><div id="elh_empresas_regiao" class="empresas_regiao">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $empresas->regiao->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($empresas->regiao->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($empresas->regiao->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($empresas->cod_empresa->Visible) { // cod_empresa ?>
		<td data-name="cod_empresa"<?php echo $empresas->cod_empresa->CellAttributes() ?>>
...
<a id="<?php echo $empresas_list->PageObjName . "_row_" . $empresas_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($empresas->nome_empresa->Visible) { // nome_empresa ?>
		<td data-name="nome_empresa"<?php echo $empresas->nome_empresa->CellAttributes() ?>>
...
<?php echo $empresas->telefone->ListViewValue() ?></span>
	...
	<?php } ?>
	<?php if ($empresas->cidade->Visible) { // cidade ?>
		<td data-name="cidade"<?php echo $empresas->cidade->CellAttributes() ?>>
<span<?php echo $empresas->cidade->ViewAttributes() ?>>
<?php echo $empresas->cidade->ListViewValue() ?></span>
</td>
	<?php } ?>
	<?php if ($empresas->regiao->Visible) { // regiao ?>
		<td data-name="regiao"<?php echo $empresas->regiao->CellAttributes() ?>>
