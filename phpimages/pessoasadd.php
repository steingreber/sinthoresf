...
$pessoas_add = NULL; // Initialize page object first

class cpessoas_add extends cpessoas {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["cod_pessoa"] != "") {
				...
				$this->setKey("cod_pessoa", $this->cod_pessoa->CurrentValue); // Set up key
			} else {
				$this->setKey("cod_pessoa", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("pessoaslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "pessoasview.php")
...
// Create page object
if (!isset($pessoas_add)) $pessoas_add = new cpessoas_add();

// Page init
$pessoas_add->Page_Init();

// Page main
$pessoas_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pessoas_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var pessoas_add = new ew_Page("pessoas_add");
pessoas_add.PageID = "add"; // Page ID
var EW_PAGE_ID = pessoas_add.PageID; // For backward compatibility

// Form object
var fpessoasadd = new ew_Form("fpessoasadd");

// Validate form
fpessoasadd.Validate = function() {
...
fpessoasadd.Form_CustomValidate = 
...
fpessoasadd.ValidateRequired = true;
<?php } else { ?>
fpessoasadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fpessoasadd.Lists["x_estado"] = {"LinkField":"x_sigla","Ajax":null,"AutoFill":false,"DisplayFields":["x_estado","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $pessoas_add->ShowPageHeader(); ?>
<?php
$pessoas_add->ShowMessage();
?>
<form name="fpessoasadd" id="fpessoasadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pessoas_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pessoas_add->Token ?>">
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fpessoasadd", "x_datacadastro", "%d/%m/%Y");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fpessoasadd.Lists["x_estado"].Options = <?php echo (is_array($pessoas->estado->EditValue)) ? ew_ArrayToJson($pessoas->estado->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fpessoasadd", "x_datanasc", "%d/%m/%Y");
</script>
<?php } ?>
</span>
